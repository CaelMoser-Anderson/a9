...
public class MazeGraph {
    ...
    public MazeGraph(GameMap map) {
        ...
        // TODO 2.1: Complete the implementation of this method. Upon return, `vertices` should
        ...
        vertices.put(loc, new MazeVertex(loc));
        Queue<IPair> toAdd = new LinkedList<>();
        toAdd.add(loc);
        while (!toAdd.isEmpty()) {
            IPair current = toAdd.poll();
            for (Direction d : Direction.values()) {
                int newI = current.i();
                int newJ = current.j();
                switch (d) {
                    case UP -> newJ = (newJ - 1 + height) % height;
                    case DOWN -> newJ = (newJ + 1 + height) % height;
                    case LEFT -> newI = (newI - 1 + width) % width;
                    case RIGHT -> newI = (newI + 1 + width) % width;
                }
                IPair next = new IPair(newI, newJ);
                if (map.types()[newI][newJ] == TileType.PATH) {
                    MazeVertex nextVertex = vertices.get(next);
                    if (nextVertex == null) {
                        nextVertex = new MazeVertex(next);
                        vertices.put(next, nextVertex);
                        toAdd.add(next);
                    }

                    double weight = edgeWeight(map.elevations()[current.i()][current.j()],
                            map.elevations()[next.i()][next.j()]);
                    MazeEdge newEdge = new MazeEdge(vertices.get(current), nextVertex, d, weight);
                    vertices.get(current).addOutgoingEdge(newEdge);
                }
            }
        }
    }
